...
function BuscadorExpedientes({
  ...
  return (
    <div ref={containerRef} className="relative">
      {casoSeleccionado ? (
        // Caso seleccionado — mostrar chip con X
        <div className="flex items-center gap-2 h-8 px-3 border border-slate-200 rounded-md bg-slate-50 text-sm">
          <span className="font-mono text-xs text-slate-700 font-medium">
            #{casoSeleccionado.numero}
          </span>
          <span className="text-slate-500 truncate max-w-[160px]">
            {casoSeleccionado.titulo.length > 25
              ? casoSeleccionado.titulo.slice(0, 25) + "..."
...
export function FiltrosAuditoria({ casos, fechasActivas }: Props) {
  ...
  const hayFiltrosExtra = accionActual !== "todos" || casoActual !== "todos"
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-4 space-y-3">
      ...
      <div className="flex items-center gap-2">
        ...
        <Popover open={calendarOpen} onOpenChange={setCalendarOpen}>
          ...
          <PopoverContent className="w-auto p-0" align="start">
            ...
            <div className="px-4 pt-3 pb-1 flex items-center gap-2 text-[11px] text-slate-500 bg-slate-50/50">
              ...
              <span>Los días resaltados indican actividad registrada en los expedientes.</span>
            </div>
            ...
            {!esRango ? (
              <Calendar
                ...
                modifiersClassNames={{
                  activa: "font-bold text-blue-600 bg-blue-50/50 underline decoration-blue-400 decoration-2 underline-offset-4"
                }}
              />
            ) : (
              <Calendar
            ...
            <div className="p-3 border-t border-slate-100 flex gap-2 flex-wrap">
              <Button size="sm" variant="ghost" className="text-xs h-7"
                onClick={() => { irADia(new Date()); setCalendarOpen(false) }}>Hoy</Button>
              <Button size="sm" variant="ghost" className="text-xs h-7"
                onClick={() => { irADia(subDays(new Date(), 1)); setCalendarOpen(false) }}>Ayer</Button>
              <Button size="sm" variant="ghost" className="text-xs h-7"
                onClick={() => {
                  const hoy = new Date()
        ...
        {!esHoy && !esRango && (
          <Button variant="ghost" size="sm" onClick={() => irADia(new Date())}
            className="text-xs text-blue-600 hover:text-blue-700 h-9 shrink-0">
            Ir a hoy
          </Button>
        )}
      </div>
      ...
      <div className="flex items-center gap-2 flex-wrap pt-1 border-t border-slate-100">
        ...
        {/* Buscador de expedientes — reemplaza el Select */}
        <BuscadorExpedientes
          casos={casos}
          casoActual={casoActual}
          onSelect={(id) => updateParams({ caso: id })}
        />

        {hayFiltrosExtra && (
          <Button variant="ghost" size="sm"
            onClick={() => updateParams({ accion: null, caso: null })}
