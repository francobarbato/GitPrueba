  ...
  return (
    <Card className="shadow-md border-slate-200 bg-white h-full">
      <CardHeader className="border-b bg-gradient-to-r from-slate-50 to-white pb-4">
        ...
        <p className="text-sm text-slate-500">Tareas ordenadas por urgencia e importancia del caso.</p>
      </CardHeader>
            ...
            data.map((item, index) => (
              <Link href={item.link} key={item.id}>
                <div className="flex items-start p-4 hover:bg-slate-50 transition group cursor-pointer relative">
                  ...
                  {/* Icono según Tipo */}
                  <div
                    className={`mt-1 p-2 rounded-full mr-3 flex-shrink-0 z-10
                    ${
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-0.5">
                      ...
                      <div className="flex gap-2">
                        ...
                        {item.tipo === "Urgente" && (
                          <Badge className="h-4 text-[9px] px-1 bg-orange-500 hover:bg-orange-600">URGENTE</Badge>
                        )}
                        {item.tipo === "Abandono" && (
                      ...
                      <span
                        className={`text-xs font-bold ${item.gravedad === "Critico" ? "text-red-600" : "text-slate-500"}`}
                      >
                        {item.tiempo}
                      </span>
                    </div>

                    <p className="text-sm font-semibold text-slate-800 line-clamp-1">{item.mensaje}</p>
