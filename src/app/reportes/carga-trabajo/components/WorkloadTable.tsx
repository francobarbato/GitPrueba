    ...
    <Card className="shadow-md border-slate-200 overflow-hidden">
      <CardHeader className="border-b bg-white pb-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            <CardTitle className="text-lg font-bold text-slate-800">Crga de abogados y eficiencia</CardTitle>
            <CardDescription>Monitor en tiempo real para la asignación inteligente de casos.</CardDescription>
          </div>
        </div>
      </CardHeader>
            ...
            <tbody className="divide-y divide-slate-100 bg-white">
              {data.map((abogado) => (
                <tr key={abogado.id} className="hover:bg-slate-50 transition duration-150">
                  ...
                  <td className="px-6 py-4 text-center text-slate-600 font-medium">{abogado.enProceso}</td>
                  <td className="px-6 py-4">
                    <div className="flex flex-col items-center gap-1">
                      ...
                      <span className="text-xs font-medium text-slate-500">{abogado.eficiencia}% Cerrados</span>
                    </div>
                  </td>
                  <td className="px-6 py-4 text-center">
                    <span
                      className={`px-3 py-1 rounded-full text-xs font-bold border ${getStatusColor(abogado.estadoCarga)}`}
                    >
