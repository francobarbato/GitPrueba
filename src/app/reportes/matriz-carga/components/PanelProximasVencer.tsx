...
function labelDias(dias: number): { texto: string; color: string } {
  ...
  if (dias === 1) return { texto: "Mañana", color: "bg-red-500 text-white" }
  if (dias <= 3) return { texto: `${dias} días`, color: "bg-orange-500 text-white" }
  return { texto: `${dias} días`, color: "bg-amber-400 text-amber-900" }
}
      ...
      <CardContent className="p-0">
        {data.length === 0 ? (
          <div className="py-6 px-4 text-center">
            <p className="text-xs text-slate-400">No tenés tareas próximas a vencer.</p>
          </div>
        ) : (
