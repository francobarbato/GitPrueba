...
export function DistribucionTipo({ data, total }: { data: { procesales: number; internas: number }; total: number }) {
  ...
  return (
    <Card className="bg-white border border-slate-200 mb-6">
      ...
      <CardContent>
        {/* Barra visual: violeta vs gris azulado para que el contraste sea claro */}
        <div className="relative h-12 rounded-lg overflow-hidden bg-slate-100 mb-4 flex">
          ...
          {data.internas > 0 && (
            <div
              className="bg-slate-500 flex items-center justify-center text-white text-sm font-bold transition-all"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="p-4 bg-slate-50 border border-slate-200 rounded-lg">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-sm font-semibold text-slate-800">Internas</span>
            </div>
            <div className="flex items-baseline gap-2">
              <span className="text-2xl font-bold text-slate-700">{data.internas}</span>
              <span className="text-sm text-slate-600">eventos ({pctInterna}%)</span>
            </div>
