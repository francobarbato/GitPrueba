...
// app/reportes/cartera-fuero/components/MatrizFueroGeneral.tsx

import { useState } from "react"
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card"
import { ChevronDown, ChevronRight, Users } from "lucide-react"

type FueroRowGeneral = {
  tipo: string
  tipoLabel: string
  cantidad: number
  pesoVolumen: number
...
export function MatrizFueroGeneral({ data }: { data: FueroRowGeneral[] }) {
    ...
    <Card className="bg-white border border-slate-200 mb-6">
      <CardHeader className="pb-3">
        <CardTitle className="text-base font-semibold text-slate-800 flex items-center gap-2">
          ...
          Distribución general por fuero
        </CardTitle>
        <p className="text-xs text-slate-500">
          Volumen y capital del estudio por tipo de causa. Click en una fila para ver distribución por etapa.
        </p>
      </CardHeader>
                      ...
                      <td className="px-4 py-3 text-center">
                        <div className="flex items-center justify-center gap-2">
                          <div className="w-16 bg-slate-200 rounded-full h-2">
                            <div className="bg-purple-500 h-2 rounded-full" style={{ width: `${Math.min(row.pesoVolumen, 100)}%` }} />
                          </div>
                        ...
                        <td colSpan={9} className="bg-slate-50 border-b border-slate-200">
                          <div className="px-8 py-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                {row.distribucionEtapas.length > 0 ? row.distribucionEtapas.map(etapa => (
                                  <div key={etapa.etapa} className="flex items-center gap-2">
                                    ...
                                    <div className="flex-1 bg-slate-200 rounded-full h-2.5">
                                      <div className="bg-purple-400 h-2.5 rounded-full" style={{ width: `${etapa.porcentaje}%` }} />
                                    </div>
                            ...
                            <div className="p-3 bg-white rounded-lg border border-slate-200">
                              <p className="text-xs font-semibold text-slate-500 uppercase mb-2">Insight del estudio</p>
                              {esDesbalanceado ? (
                                <p className="text-sm text-slate-700">
                                  {row.pesoVolumen > pesoCapital ? (
                                    <><span className="font-medium text-amber-600">⚠ Alto volumen, bajo valor.</span>{" "}
                                    {row.tipoLabel} representa el {row.pesoVolumen}% del trabajo del estudio pero solo el {pesoCapital}% del capital.</>
                                  ) : (
                              ...
                              {row.tasaActividad < 40 && (
                                <p className="text-sm text-red-600 mt-2">
                                  🔴 Baja actividad en el estudio: solo el {row.tasaActividad}% con movimiento en 30 días.
                                </p>
                              )}
                            </div>
                          </div>
