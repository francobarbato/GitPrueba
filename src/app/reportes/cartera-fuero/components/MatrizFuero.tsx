    ...
    <Card className="bg-white border border-slate-200 mb-6">
      <CardHeader className="pb-3">
        ...
        <p className="text-xs text-slate-500">
          Cruce de volumen de trabajo vs. valor económico por materia. Click en una fila para ver distribución por etapa.
        </p>
      </CardHeader>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {data.map((row, idx) => {
                ...
                // Detectar si volumen y valor están desbalanceados
                  ...
                  <> 
                    ...
                    >
                        ...
                        <div className="flex items-center justify-center gap-2">
                          <div className="w-16 bg-slate-200 rounded-full h-2">
                            <div
                              ...
                              style={{ width: `${Math.min(row.pesoVolumen, 100)}%` }}
                            />
                          </div>
                      ...
                      <td className="px-4 py-3 text-right font-medium text-slate-700">
                        {formatMoney(row.capitalEnLitigio)}
                      </td>
                      <td className="px-4 py-3 text-center">
                        <div className="flex items-center justify-center gap-2">
                          <div className="w-16 bg-slate-200 rounded-full h-2">
                            <div
                              ...
                              style={{ width: `${Math.min(pesoCapital, 100)}%` }}
                            />
                          </div>
                      ...
                      <td className="px-4 py-3 text-right font-medium text-slate-700">
                        {formatMoney(row.ticketPromedio)}
                      </td>
                      <td className="px-4 py-3 text-center">
                        <span
                          className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${
                            row.tasaActividad >= 70
                    ...
                    {/* Fila expandida: distribución por etapa */}
                    {isExpanded && (
                      <tr key={`${row.tipo}-detail`} className="bg-slate-50">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                              ...
                              <div className="space-y-2">
                                {row.distribucionEtapas.length > 0 ? (
                                  row.distribucionEtapas.map((etapa) => (
                            ...
                            <div className="p-3 bg-white rounded-lg border border-slate-200">
                                ...
                                <p className="text-sm text-slate-700">
                                  {row.pesoVolumen > pesoCapital ? (
                                    <>
                                      ...
                                      {row.tipoLabel} representa el {row.pesoVolumen}% del trabajo pero solo el{" "}
                                      {pesoCapital}% del capital. Consume recursos sin retorno proporcional.
                                    </>
                                  ) : (
                                    <>
                                      <span className="font-medium text-emerald-600">✦ Alta rentabilidad.</span>{" "}
                                      {row.tipoLabel} solo es el {row.pesoVolumen}% del volumen pero concentra el{" "}
                                      {pesoCapital}% del capital. Fuero estratégico para el estudio.
                                    </>
                                  )}
                                </p>
                              ) : (
                              ...
                              {row.tasaActividad < 40 && (
                                <p className="text-sm text-red-600 mt-2">
                                  🔴 Baja actividad: solo el {row.tasaActividad}% de los casos tuvieron movimiento en los últimos 30 días.
                                </p>
                              )}
                            </div>
                          </div>
