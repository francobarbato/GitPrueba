...
/**
 * TASA DE CUMPLIMIENTO — definición única del sistema
 *
 * Un evento "cumple el plazo" si fue completado en o antes de su fecha de vencimiento.
 * Todo lo demás (completado con demora, vencido sin completar) NO cumplió el plazo.
 *
 * Por eso el denominador incluye las tres categorías: en plazo + con demora + vencidas.
 * El bug previo calculaba "enPlazo / completadas" ignorando las vencidas, lo que
 * producía tasas artificialmente altas (p.ej. 100% con tareas vencidas presentes).
 */
...
/**
 * Días de ejecución con granularidad sub-diaria.
 * Si la tarea se creó y completó el mismo día (< 24hs de diferencia),
 * devolvemos un marcador especial para que la UI muestre "<1d" en vez de "0d".
 */
function calcularDiasEjecucion(createdAt: Date, completadaAt: Date): number | "<1" {
  ...
  if (horas < 0) return "<1" // fallback seguro si hay datos raros
  if (horas < 24) return "<1"
  return differenceInDays(completadaAt, createdAt)
}

// ============================================================================
// QUERY PRINCIPAL — enfocada en PASADO (cómo se trabajó)
// ============================================================================
...
async function getCumplimientoTareas(periodoDias?: number, abogadoId?: string) {
  ...
  // ========== DETALLE DE EVENTOS PARA TABS (vista personal) ==========
  // Mapea cada evento a una forma serializable para el componente.
  // diasEjecucion puede ser un número (días), el string "<1" (menos de 24hs), o null (no completado).
  ...
  // ========== POR PERSONA — solo métricas de resultado ==========
  ...
  for (const t of tareas) {
    // Solo contar eventos terminales (COMPLETADA o VENCIDA)
  ...
  const porPersona = Array.from(porPersonaMap.values()).map(p => ({
    ...
    // FIX: incluye vencidas en el denominador para una tasa honesta
  ...
  // ========== POR PRIORIDAD — métricas de resultado ==========
  ...
  const porPrioridad = prioridades.map(prio => {
    ...
    return {
      ...
      // FIX: incluye vencidas en el denominador
  ...
  // ========== DATOS RAW PARA RECALCULO CLIENT-SIDE (vista gerencial) ==========
  ...
  // ========== INSIGHTS ==========
  // Requerimos >= 2 completadas para considerar a alguien en el ranking
  // (evita insights de personas con 1 sola tarea con 100% o 0%)
      ...
      <div className="flex flex-col flex-1 overflow-hidden">
        ...
        <main className="flex-1 overflow-auto p-6">
          <div className="max-w-7xl mx-auto">
            ...
            {/* Detalle por tabs — solo vista personal */}
