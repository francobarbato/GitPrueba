...
async function getCumplimientoTareas(periodoDias?: number, abogadoId?: string) {
  ...
  // ========== DETALLE DE TAREAS PARA TABS (vista personal) ==========
  const mapearDetalle = (t: TareaRaw) => {
    const diasEjec = t.fechaCompletada ? differenceInDays(t.fechaCompletada, t.createdAt) : null
    ...
    return {
      ...
      diasEjecucion: diasEjec !== null && diasEjec >= 0 ? diasEjec : null,
  ...
  for (const t of tareas) {
    // Solo contar tareas terminales (COMPLETADA o VENCIDA)
    ...
    if (t.estado === "COMPLETADA") {
      ...
      if (t.fechaCompletada) { const dias = differenceInDays(t.fechaCompletada, t.createdAt); if (dias >= 0) p.diasEjecucion.push(dias) }
  ...
  const porPersona = Array.from(porPersonaMap.values()).map(p => ({
    ...
    tasaCumplimiento: p.completadas > 0 ? Math.round((p.enPlazo / p.completadas) * 100) : 0,
...
export default async function CumplimientoTareasPage({ searchParams }: PageProps) {
  ...
  const subtitulo = vistaGeneral
    ...
    : periodoParam && PERIODO_LABELS[periodoParam] ? `Mis resultados — ${PERIODO_LABELS[periodoParam]}` : "Análisis de mis tareas completadas y su cumplimiento"
          ...
          <div className="max-w-7xl mx-auto">
            <div className="flex items-center justify-between mb-4 flex-wrap gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2"><ClipboardCheck className="h-6 w-6 text-blue-600" />Cumplimiento de tareas</h1>
            ...
            <div className="mt-8 p-4 bg-slate-100 border border-slate-200 rounded-lg">
              ...
              <p className="text-xs text-slate-500">
                <strong>Enfoque histórico:</strong> Este reporte analiza tareas terminadas (completadas y vencidas), no tareas activas.{" "}
                <strong>En plazo:</strong> Completadas antes o en la fecha de vencimiento.{" "}
                <strong>Con demora:</strong> Completadas después de la fecha de vencimiento.{" "}
                <strong>Vencidas:</strong> Tareas cuyo plazo expiró sin completarse.{" "}
