...
export function PanelInsightsTareas({ data, vistaGeneral }: { data: InsightsData; vistaGeneral: boolean }) {
  ...
  if (vistaGeneral && data.mejorCumplimiento) {
    insights.push({
      icon: Award,
      text: `${data.mejorCumplimiento.nombre} tiene la mejor tasa de cumplimiento en plazo con ${data.mejorCumplimiento.tasa}% y un promedio de ${data.mejorCumplimiento.promDias} días por tarea.`,
      color: "bg-emerald-50 border-emerald-200",
    })
  }

  if (vistaGeneral && data.peorCumplimiento) {
    insights.push({
      icon: TrendingUp,
  ...
  if (data.prioridadMasRapida) {
    insights.push({
      icon: Clock,
      text: `Las tareas de prioridad "${PRIORIDAD_LABELS[data.prioridadMasRapida.prioridad] ?? data.prioridadMasRapida.prioridad}" se resuelven más rápido, con ${data.prioridadMasRapida.dias} días promedio de ejecución.`,
  ...
  if (data.totalVencidas > 0) {
    insights.push({
      icon: AlertTriangle,
      text: `Hay ${data.totalVencidas} tarea${data.totalVencidas !== 1 ? "s" : ""} vencida${data.totalVencidas !== 1 ? "s" : ""} sin completar que requiere${data.totalVencidas !== 1 ? "n" : ""} atención del equipo.`,
    ...
    <Card className="bg-white border border-slate-200 mb-6">
      <CardHeader className="pb-3">
        ...
        <p className="text-xs text-slate-500">
          Conclusiones calculadas a partir de los datos de tareas del estudio.
        </p>
      </CardHeader>
