          ...
          <tbody>
            {data.map((p, idx) => (
              <tr key={p.id} className={`border-b border-slate-100 ${idx % 2 === 0 ? "bg-white" : "bg-slate-50/50"}`}>
                ...
                <td className="px-3 py-3 text-center font-medium text-slate-700">{p.total}</td>
  ...
  return (
    <Card className="bg-white border border-slate-200 mb-6">
      <CardHeader className="pb-3">
        ...
        <p className="text-xs text-slate-500">Resultado histórico de eventos resueltos por cada miembro del equipo.</p>
      </CardHeader>
