...
// diasEjecucion puede ser:
//   - number: cantidad de días completos
//   - "<1": menos de 24 horas (creación y completado el mismo día)
//   - null: no completado o sin fecha
type DiasEjecucion = number | "<1" | null
...
// ============================================================================
// HELPER: formateo de días de ejecución
// ============================================================================

function formatDiasEjecucion(d: DiasEjecucion): string {
  if (d === null) return "—"
  if (d === "<1") return "<1d"
  return `${d}d`
}
    ...
    <tr className="border-b border-slate-100 hover:bg-slate-50/50 transition-colors">
      ...
      <td className="px-3 py-2.5 text-center">
        {tipoTab === "vencidas" && t.diasAtraso !== null ? (
          ...
          <span className="text-xs text-slate-500">{formatDiasEjecucion(t.diasEjecucion)}</span>
        ) : (
          <span className="text-xs text-slate-400">—</span>
        )}
      </td>
    ...
    <Card className="bg-white border border-slate-200 mb-6">
      <CardHeader className="pb-3">
        <CardTitle className="text-base font-semibold text-slate-800 flex items-center gap-2">
          ...
          Detalle de eventos resueltos
        </CardTitle>
        <p className="text-xs text-slate-500">Listado de los eventos que conforman cada KPI. Hacé click en un tab para ver los eventos correspondientes.</p>
      </CardHeader>
      <CardContent className="p-0">
        ...
        {tareasActivas.length === 0 ? (
          <p className="text-sm text-slate-400 text-center py-8">{hayFiltros ? "Sin eventos para los filtros seleccionados" : "Sin eventos en esta categoría"}</p>
        ) : (
              ...
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200">
                  ...
                  <th className="text-center px-3 py-2.5 text-xs font-semibold text-slate-500 uppercase tracking-wider">
                    {tabActivo === "vencidas" ? "Venció" : "Completado"}
                  </th>
