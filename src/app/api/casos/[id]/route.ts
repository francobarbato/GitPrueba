...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    // ABOGADO solo puede ver sus propios casos o colaboraciones
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    // ADMIN no puede editar casos
    if (userRol === 'ADMIN') {
      return NextResponse.json({ error: "El administrador no puede editar casos" }, { status: 403 })
    }

    // ASISTENTE no puede editar casos por API
    if (userRol === 'ASISTENTE') {
      return NextResponse.json({ error: "No tenés permiso para editar casos" }, { status: 403 })
    }

    // ABOGADO solo puede editar sus propios casos
    if (userRol === 'ABOGADO') {
