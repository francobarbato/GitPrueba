              ...
              {!casoPreseleccionado && (
                <Combobox label="Expediente asociado" placeholder="Sin expediente vinculado"
                  ...
                  <div className="max-h-[280px] overflow-y-auto p-1">
                    ...
                    {casosFiltrados.map(c => (<div key={c.id} onClick={() => { setCasoId(c.id); setOpenCaso(false); setSearchCaso("") }} className={`flex w-full cursor-pointer items-center rounded-md py-2.5 px-3 text-sm hover:bg-slate-100 transition-colors ${casoId === c.id ? "bg-blue-50 border border-blue-200" : ""}`}><span className="font-mono text-xs text-blue-600 font-bold mr-2 shrink-0">{c.numero}</span><span className="truncate">{c.titulo}</span></div>))}
