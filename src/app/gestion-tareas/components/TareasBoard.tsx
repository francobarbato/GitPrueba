...
import {
  ...
  marcarTareasComoVistasAction, getTareaDetalle, cerrarVencidaAction
...
function GrupoTareas({ titulo, icono: Icono, colorTitulo, tareas, onChange, onEdit, onDelete, onOpenDrawer, onCerrarVencida, soloLectura, currentUserId, ultimoAcceso }: {
  titulo: string
  icono: any
  colorTitulo: string
...
function SeccionTareas({ titulo, tareas, colorTitulo, onChange, onEdit, onDelete, onOpenDrawer, onCerrarVencida, soloLectura, currentUserId, ultimoAcceso, agruparPorDisponibilidad = false }: {
  titulo: string
  tareas: TareaConRelaciones[]
  colorTitulo: string
  onChange: (id: string, estado: EstadoTarea, motivo?: string) => void
...
useEffect(() => {
  ...
  // ⚠️ SOLO evitar si el drawer YA está mostrando esa tarea
  if (lastOpenedRef.current === tareaAbierta && drawerTarea?.id === tareaAbierta) {
    return
  }

  lastOpenedRef.current = tareaAbierta

  const abrirDrawer = (t: TareaConRelaciones) => {
    setDrawerTarea(null)
    setTimeout(() => setDrawerTarea(t), 0)
  }

  const enMemoria = tareas.find(t => t.id === tareaAbierta)

  if (enMemoria) {
    abrirDrawer(enMemoria)
  } else {
...
}, [searchParams, tareas, drawerTarea]) // ← se removió `tareas` de las deps
  ...
  const tareasConNovedad = useMemo(() =>
    tareas.filter(t =>
      ...
      getTipoNovedad(t, ultimoAcceso, currentUserId) !== null &&
  ...
  const kpisVisibles = 2 + (fatalesCount > 0 ? 1 : 0) + (vencidasAbiertasCount > 0 ? 1 : 0)
  const gridClase =
  ...
  return (
    <>
          ...
          {fatalesCount > 0 && (
            <button
              ...
              className={`bg-red-50 border rounded-xl p-4 text-left transition-all hover:shadow-md cursor-pointer ${
                filtroKpi === 'fatales' ? 'ring-2 ring-red-500 border-red-500' : 'border-red-200'
              }`}
            >
              <div className="flex items-center gap-2 mb-1">
              ...
              <p className="text-3xl font-bold text-red-700">{fatalesCount}</p>
            </button>
          )}
      ...
      {(tipoFiltro === "TODOS" || tipoFiltro === "PROCESAL") && <SeccionTareas titulo="Procesales" tareas={tipoFiltro === "PROCESAL" ? tareasFiltradas : procesales} colorTitulo="text-red-700" onChange={handleCambioEstado} onEdit={handleEdit} onDelete={handleDelete} onOpenDrawer={handleOpenDrawer} onCerrarVencida={handleCerrarVencida} soloLectura={vistaActiva === "supervisadas"} currentUserId={currentUserId} ultimoAcceso={ultimoAcceso} agruparPorDisponibilidad={vistaActiva === "mias"} />}
      {(tipoFiltro === "TODOS" || tipoFiltro === "INTERNA") && <SeccionTareas titulo="Internas" tareas={tipoFiltro === "INTERNA" ? tareasFiltradas : internas} colorTitulo="text-blue-700" onChange={handleCambioEstado} onEdit={handleEdit} onDelete={handleDelete} onOpenDrawer={handleOpenDrawer} onCerrarVencida={handleCerrarVencida} soloLectura={vistaActiva === "supervisadas"} currentUserId={currentUserId} ultimoAcceso={ultimoAcceso} agruparPorDisponibilidad={vistaActiva === "mias"} />}
