...
import { Eye, EyeOff, Loader2, UserCheck, Check, X } from "lucide-react";
import { confirmarActivacionCuenta } from "src/lib/actions/account-activation-actions";
import { PasswordRequirements } from "src/app/components/auth/PasswordRequirements";
...
export function ActivateAccountForm({ token, email, nombre, apellido }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    const res = await confirmarActivacionCuenta({
      token,
      password,
    ...
    window.location.href = "/auth/signin?activated=success";
  };
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-slate-600 mt-2 text-sm">
              Para activar tu cuenta <strong>{email}</strong>, elegí una contraseña segura.
            </p>
          </div>
