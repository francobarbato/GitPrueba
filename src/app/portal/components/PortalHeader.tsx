...
import {
  Scale, User, LogOut, ChevronDown,
  Home, FileText, Bell, Settings,
  Clock, MapPin, ArrowRight, CheckCircle2, CheckCheck
} from 'lucide-react'
import { signOut } from 'next-auth/react'
import { getNotificacionesPortalAction, marcarPortalComoVistoAction } from "src/lib/actions/portal/portal-tarea-actions"
import type { TareaPortal } from "src/lib/actions/portal/portal-tarea-actions"
...
export function PortalHeader({ user }: PortalHeaderProps) {
  ...
  const [tareas, setTareas] = useState<TareaPortal[]>([])
  ...
  useEffect(() => { if (sheetOpen) cargarNotificaciones() }, [sheetOpen])

  const cargarNotificaciones = () => {
    startTransition(async () => {
      try {
        const data = await getNotificacionesPortalAction()
        setTareas(data.tareas)
        setTotalNuevas(data.totalNuevas)
      } catch (e) {
        console.error("Error cargando notificaciones del portal:", e)
      }
    })
  }
  ...
  const handleMarcarVistas = () => {
    startTransition(async () => {
      const result = await marcarPortalComoVistoAction()
      if (result.success) {
        setTareas([])
        setTotalNuevas(0)
        setSheetOpen(false)
            ...
            <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
              ...
              <SheetContent className="w-full sm:w-[400px] p-0 flex flex-col">
                <SheetHeader className="px-5 pt-5 pb-4 border-b border-slate-100 shrink-0">
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">Tareas nuevas o actualizadas por su estudio</p>
                  {totalNuevas > 0 && (
                    <button
                ...
                <div className="flex-1 overflow-y-auto">
                  {tareas.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
                      ...
                      <p className="text-xs text-slate-400 mt-1">No hay tareas nuevas desde su última visita</p>
                    </div>
                  ) : (
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-56">
                ...
                <DropdownMenuItem asChild>
                  <Link href="/portal/perfil" className="flex items-center cursor-pointer">
